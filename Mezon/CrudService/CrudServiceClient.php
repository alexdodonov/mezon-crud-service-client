...
class CrudServiceClient extends ServiceClient implements CrudServiceClientInterface
{
    ...
    {
        $rewriteMode = $this->getRewriteMode();

        $urlMap = [
            'fields' => $rewriteMode ? '/fields/' : '?r=fields',
            'list' => $rewriteMode ? '/list/?' : '?r=list&',
            'deleteFiltered' => $rewriteMode ? '/delete/?' : '?r=delete&',
            'recordsCountByField' => $rewriteMode ? '/records/count/' . $param . '/?' : '?r=' .
            urlencode('records/count') . '&',
            'delete' => $rewriteMode ? '/delete/' . $param . '/?' : '?r=' . urlencode('delete/' . $param) . '&',
            'lastRecords' => $rewriteMode ? '/last/' . $param . '/?' : '?r=' . urlencode('last/' . $param) . '&',
            'recordsCount' => $rewriteMode ? '/records/count/' : '?r=' . urlencode('records/count'),
